...
@section('content')
        ...
        <ol class="breadcrumb">
            ...
            @foreach ($truyen->thuocnhieudanhmuctruyen as $key => $breadcrumb_danh)
                <li class="breadcrumb-item"><a
                        href="{{ url('danh-muc/' . $breadcrumb_danh->slug_danhmuc) }}">{{ $breadcrumb_danh->tendanhmuc }}</a>
                </li>
            @endforeach
    ...
    <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-md-7">
                    ...
                    <ul class="infotruyen">
                        ...
                        <li>Ngày đăng : <span class="text text-primary">{{ $truyen->created_at->diffForHumans() }}</span>
                        </li>
                        <li>Loại truyện :
					...
					<li>Thể loại truyện : <a href="{{url('the-loai/'.$truyen->theloai->slug_theloai)}}">{{$truyen->theloai->tentheloai}}</a></li> --}}
                        ...
                        @if ($truyen->loaitruyen != 'truyentranh')
                            @if ($chapter_dau)
                                <li><a href="{{ url('xem-chapter/' . $chapter_dau->truyen->slug_truyen . '/' . $chapter_dau->slug_chapter) }}"
                                        class="btn btn-primary">Đọc Truyện</a>
                                    <button class="btn btn-danger btn-thich_truyen"><i class="fa fa-heart"
                                            aria-hidden="true"></i> Thích truyện</button>
                                </li>

                                <li><a href="{{ url('xem-chapter/' . $chapter_moinhat->truyen->slug_truyen . '/' . $chapter_moinhat->slug_chapter) }}"
                        ...
                        @else
                            @if ($chapter_dau)
                                <li><a href="{{ url('xem-truyen-tranh/' . $chapter_dau->truyen->slug_truyen . '/' . $chapter_dau->slug_chapter) }}"
                                        class="btn btn-danger">Xem Truyện Tranh</a>
                                    <button class="btn btn-danger btn-thich_truyen"><i class="fa fa-heart"
                                            aria-hidden="true"></i> Thích truyện</button>
                                </li>
                ...
                @if ($mucluc > 0)
                    @foreach ($chapter as $key => $chap)
                        @if ($chap->loaichapter != 'truyentranh')
                            <li><a
                                    href="{{ url('xem-chapter/' . $chap->truyen->slug_truyen . '/' . $chap->slug_chapter) }}">{{ $chap->tieude }}</a>
                            </li>
                        @else
                            <li><a
                                    href="{{ url('xem-truyen-tranh/' . $chap->truyen->slug_truyen . '/' . $chap->slug_chapter) }}">{{ $chap->tieude }}</a>
                            </li>
                        @endif
                    @endforeach
                @else
        ...
        <div class="col-md-3">
            ...
            @foreach ($truyennoibat as $key => $noibat)
                <div class="row mt-2">
                    ...
                    <div class="col-md-5" id="d"><img class="img img-responsive" width="100%" class="card-img-top"
                            src="{{ asset('public/uploads/truyen/' . $noibat->hinhanh) }}"
            ...
            <h3 class="card-header">Được xem nhiều</h3>
